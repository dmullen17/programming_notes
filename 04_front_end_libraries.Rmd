...
### Self-closing tags
In html we can use some elements that don't include a closing tag like `<br>`.  In JSX all elements must either be followed by a closing tag or be written in self-closing format `<br>` --> `<br />`.  
```
const JSX = (
  <div>
...
#### Composition
We can put multiple components together into one - for instance navbar, dashboard, and footer components.  To compose then together we create a parent component that calls then.  They are called using self-closing JSX tag syntax. 
```
return (
<App>
  <Navbar />
...
#### Default Props 
We can set default properties using the following syntax.  These are good for initial values. 
```
...
### &&
`&&` allows you to evaluate an expression and if true it can render the subsequent JSX markup.  This allows us to chain together more complex if/else blocks in a more concise manner.  