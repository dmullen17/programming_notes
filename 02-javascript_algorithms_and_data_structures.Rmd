...
+ Sorting examples. Sorting defaults to unicode which converts numbers to characters.  We need callback functions to sort numeric values correctly.  Sort also mutates the original array.  
```
...
// Returns ['z', 's', 'l', 'h', 'b']
```
+ `.join('delimiter')` joins elements of an array using a delimiter 
+ `.every` takes a callback function and checks if every value is true.  
    +`.some` (any in R) method checks if any applications of the callback function are true
