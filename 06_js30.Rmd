...
### center li
The following CSS will center `<li>` elements by removing their bullets points and the default value of `padding-right: 40px`.

```
ul {
    list-style-type: none;
    padding: 0; /* Centers the <li> with its box*/
}
```

### transform: perspective
When we use transform to rotate an element we want to add some perspective.  For instance if we rotate on its X-axis by 3deg we can't really see this if we're too close or too far away.  A perspective of 100-200 px lets use see the rotation nicely.  

```
li:nth-child(even) {
    transform: perspective(100px) rotateX(45deg) translateY(3px);
    background: linear-gradient(to top,  #ffffff 0%,#EFEFEF 100%);
}
```

### linear-gradient to top
Linear-gradient has options for specifying which way you want the gradient to go.  Here we use `to-top` and `to-bottom` to create a nice repeating pattern on odd and even child elements.
...
This setting pushes items apart.  If there are two items they will be aligned against the left and right walls of their container.

### Summary 
The first thing we do is fetch a list of cities and states using the fetch API. fetch(endpoint) returns a promise, which we use `.then` on the parse the blob of data is returns. We then call `.then` onces more to push the resulting array to our cities array. Next we create an input box with an unordered list.  We listen for a keyup event in the input element.  Our callback function first uses a RegExp object with `this.value` (the input that the user typed), and filters the list of cities down using the `match` method.  We then use the `map` method to return an `<li>` element for each combination of city, state, and population.  We wrap the city and state in a span, and the population in another span with different classes - for custom formatting.  Finally we use the `replace` method on city, and state to replace the regexp pattern with a span of class 'highlight' that wraps `this.value` in order to highlight the user's search in the results.  This is then set to one string using `join` and set to the `innerHTML` of the `ul`.
...
### querySelector on attributes
We can call `querySelector` to search for DOM elements with non-null attributes fields.  For instance here select all attributes with populated `data-skip` attributes.
```
const skipButtons = player.querySelectorAll('[data-skip]');
```

### play pause event
The video element has `pause` and `play` events that fire when the video transitions from each state.  Here is a function and two event listeners to illustrate it. 
...
### timeupdate event
The html5 video element has a `timeupdate` event that fires when the video updates it's time. There are some differences between browsers: Firefox fires the timeupdate event once per frame. Safari 5 and Chrome 6 fire every 250ms. Opera 10.50 fires every 200ms.

### mousedown flag 
On our scrubbing progress bar we want to be able to click and to hold the mousedown and drag to a point on it.  To do this we need a flag variable, we initialize it to false, and set it to true when we catch a mousedown event on the progress bar element, while it is true the 'mousemove' event listener will activate because it uses the mousedown flag variable in its callback function.  When we mouseup another event listener sets the flag to false. 
...
### fullscreen
There is a method called `requestFullScreen` in some variation depending on browser.  The details are below:
...
### position: fixed
When we transition an element from position on a page to `fixed` the content can "jump" up - this is because the element is no longer up space on the page so stuff below it moves up accordingly. This is called a "reflow".  We need to offset this using padding.

## Event propogation, capture, and bubbling.  

When we click add an event listener for a click the event will bubble up through the DOM and trigger the callback function for each subsequent element.

### Capture 
A capture is when an event occurs - it will start at the top then go to the body, a subsequent, etc. until it reaches the stopping point.  It "captures" the event at each of these places but it hasn't fired yet. Then it will fire the event back up through the chain it just went down. In the `addEventListener` function the third arugment is set to `capture: false` by default.  If we set it to `{capture: true}` (remember it's an object) then it will run the callback function on the way down (html -> body -> etc.) rather than up.

### e.stopPropogation()
Using this function in a callback function will stop the event from bubbling up.

### once  
In the third argument of an event listener we can set ``{capture: true, once: true}` - which will unbind itself.  This is equivalent to `element.removeEventListener('click', callBack)`. 
...
This function returns the bounding coordinates of a DOM element.

```
const linkCoords = this.getBoundingClientRect();
console.log(linkCoords);
DOMRect {x: 538.78125, y: 10, width: 123.59375, height: 18, top: 10, …}
bottom: 28
height: 18
left: 538.78125
right: 662.375
...
### why does display: block show it?
The span only shows up when you have `display: block` enabled in its CSS.  Need to investigate this further.  -- This is probably not the case. 

### z-index
z-index only works on positioned elements (position:absolute, position:relative, or position:fixed). You also cannot apply z-indexes to backgrounds.  So if I want to use the z-index technique in this tutorial I set it to 0 (so it's in front of backgrounds), and then z-indexes on other elements to higher numbers (1, 2, etc.).  

### Callback function arguments 
Callback functions do not accept arguments.  there are two strategies to circumvent this.  We can either use a closure or use bind.  
...
### Sz-index: -1 
If you set z-index: -1 then certain events like 'mouseenter' and 'click' will not trigger on the element.

### ummary
In this project we create a basic webpage. When we hover over anchor tags there is a <span> element that redirects behind these. It has a transition animation so we know it's only one <span> that moves around. We initialize the span element to the top of the page with height and width 0.  Next we add an event listener to each anchor tag with a callback function. The callback function uses `this.getBoundingClientRect()` to get the height, width, and x,y coordinates of the anchor tag.  We reset the span's height, width, and transform properties to these.  We also need to make a scroll adjustment - if we scroll then x,y coordinates will be off by the scroll - so we add these in to the `transform` property.  Finally the span's z-index is set to 0. This is so it will appear in front of backgrounds, but behind other elements with higher z-indices.  It's also important to note that in order to use a z-index property an element must also have a `position` property.
...
### whitespace
`whitespace: nowrap` is the property that makes `display: inline-flex` elements stay on the same line. 
...
### user-select 
In web browers if you double click some elements their text gets highlighted.  We can set `user-select: none` in order to prevent this. 

### perspective 
The `persepective` property is used is used to give a 3D element some persepective.  When defining a perspective property it's the child elements that actually get the perspective.  We need to use `perspective` in conjunction with another property.  Here we use it with `transform: scaleX(1.31) rotateY(40deg);`.  We get a 500px perspective, and as we scroll we see the element rotate from 0 - 40 deg in the Y-axis direction.  Similarly we also see the X-axis resize.


### scaleX 
This property does exactly what it sounds like.  If you pass `scaleX(2)` it will double the length of the X-axis of the element it applies to.  

### will-change
Lets the browser know how an element is expected to change so it can optimize the behavior.  The browser already tries to optimize things so this should be used sparingly.

### font-size: 0
There is some whitespace between inline-block elements by default.  One way to remove this white space is to set `font-size: 0`.  There are other options listed here: https://css-tricks.com/fighting-the-space-between-inline-block-elements

### e.pageX - slider.offsetLeft
With this we can figure out where in an element we are.  `e.pageX` will normally return this on a mouse event, however if there is a margin it doesn't account for that.  So in this case `slider` is the element with the event listener, so we subtract its margin `slider.offsetLeft` (if any exists).
```
e.pageX - slider.offsetLeft
```

### console.log({x, startX})
If we wrap the input of a console.log call in an object. It will output it in the following pretty-printed format: ```{x: 524, startX: 239}```

### Summary
In this project we put x-number of divs side by side in a flex container.  We use the `display: inline-flex` and `overflow-x` properties to move them off the page.  The `perspective` and `transform` properties give them depth, and if you scroll these update based on where it is in the page. The JS logic uses `mouseup` and `mouseexit` as events that set our scrolling flag to `false`, `mousedown` sets it to true.  Next we define an initial variable `lastScreenX` and set it to 0.  If it's set to 0 our callback function will set it to the `event.clientX` and return.  If it's not set to 0 then it will set `this.scrollLeft += (event.clientX - lastScreenX)*-1` - which is the offset of how much we scrolled between two mousemove events.  We need to multiply by -1 because moving your mouse right should cause content to scroll left.

### Wes's App logic 
Wes used essentially the same logic as I did.  The event listeners were the same. Instead of continuously updating `slider.scrollLeft` by using +=, we logs its value and and the `startX` value on each `mousedown` event to reset the origins.  This is probably better than logic than I used by resetting it to 0 each time.  
...
### overflow: hidden
The scrubber uses a `div` within another `div` to create the filled effect.  The parent div has a border-radius, but we don't want to apply that to the child because we want the bottom of the child to be a straight line.  The solution is to use `overflow: hidden` on the parent. Which will hide the outer edges of the child rectangle that exceed the parent's boundary. 

### .toPrecision(1)
We can call this method on a float to round it a certain number of significant digits. 
...
The `mouseout` event can be frustrating because it fires when the cursor leaves the selected element AND its child elements.  The `mouseleave` event is a good alternative to this because it only fires when the mouse leaves the selected element.
...
### setTimeout
When you use a setTimeout, the code that follows it is not held up and waiting for the delay in time before it fires. It just fires regardless if the the setTimeout delay is done. So using a traditional for loop to fire off sequential setTimeout functions is no good. The loop will fire off all it’s iterations before the delay has finished firing once, preventing it from ever firing more than that one time.

### timer function 
Wes uses the same logic as I did, but he defines the global `countdown` variable so the function can be referenced outside the scope. He also creates a `then` variable which is the time the timer should stop in the future.  Each second we get the current time with the `Date.now()` method, and re-calculate how many seconds are remaining. 
...
### document.title 
This is the DOM element that controls the `<title>` tag in the tab of a webpage.  We can dynamically update it to set it equal to the time remaining. 

### Date.now()
returns the number of milliseconds since Januaray 1st, 1970;

### document.name 
If a DOM element has a name attribute you can select it with `document.name`. If that element has children with name attributes you can then select those `document.name.name`.  
...
```
We can select this DOM element with the following JS
```
document.customForm.minutes
```

## Stripe Follow Along
Based on the CSS it looks when our mouse enters a `div` of class `dropdown` it will change the `display` property from `none` to `block`. Finally a div of class `dropdownbackground` follows the cursor around and cahnges to `opacity: 1` when it's active.

+ cms: content management system 

### display: none;
This property hides elements from the page, although they are there.

## Arrow function vs. normal inheriting this 
Arrow functions inherit the value of `this` from their parents while normal functions do not.  In this example `this` corresponds to an `<li>` element. In the first `setTimeout`, this refers to the window and it doesn't set the classList properly.  In the second one we use an arrow function and this corresponds to the `<li>` element. 
